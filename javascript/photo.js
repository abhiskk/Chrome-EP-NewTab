...
backgrounds.Photo = new Model({
  ...
  isValidLocalStorageState: function(cacheSize) {
    var count = localStorage.getIem("count");
    var order = JSON.parse(localStorage.getItem("order"));
    var cachedImages = JSON.parse(localStorage.getItem("cachedImages"));
    return count != null && count >= 0 && order != null && order.length == this.BACKUP_IMAGES_COUNT && cachedImages != null && cachedImages.length <= cacheSize;
  },
  ...
  display: function(numberOfImages, cacheSize) {
    ...
    var order = JSON.parse(localStorage.getItem("order");
      ...
      if (count < this.STARTUP_IMAGE_COUNT || cachedImages.length == 0) {
        var order_index = count % this.STARTUP_IMAGE_COUNT;
      ...
      if (localStorage.getItem("cachedImages").length < cacheSize) {
